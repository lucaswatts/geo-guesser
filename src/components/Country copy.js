...
class Country extends React.Component {
  ...
  startGame() {
    ...
    this.setState({
      randNum: Math.floor(Math.random() * Countries.data.length),
    });
  ...
  newCity() {
    ...
    this.setState({
      randNum: (this.state.randNum =
        Math.floor(Math.random() * Countries.data.length) + 1),
    });
    console.log("game starting");
    cityDisplay.innerHTML = Countries.data[this.state.randNum].City;
  }
  ...
  skip() {
    ...
    displaySkipped.innerHTML = `${
      Countries.data[this.state.randNum].City
    } is the capital city of ${Countries.data[this.state.randNum].Country}.`;

    this.setState({
      randNum: (this.state.randNum = Math.floor(
        Math.random() * Countries.data.length + 1
      )),
    });
